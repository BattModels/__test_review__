...
from .energy import sampling, convex_hull, CommonTangent, calculate_S_config_total
...
def _convert_LFP_data_to_dsdx(datafile_name="LFP_dSdx_modified.csv", T=300):
    working_dir = os.getcwd()
    df = pd.read_csv("LFP_dSdx_modified.csv",header=None)  # raw data unit is J/(mol.K)
    data = df.to_numpy()
    x_measured = data[:,0] # Li filling fraction, from 0 to 1
    dsdx_measured = data[:,1]
    # convert to torch.tensor
    x_measured = x_measured.astype("float32")
    x_measured = torch.from_numpy(x_measured)
    dsdx_measured = dsdx_measured.astype("float32")
...
# def calc_loss_entropy(x_measured, dsdx_measured, S_config_params_list, n_list, Theta_E_list, T_dsdx=320, style = "Legendre"):
...
#         s_vib_tot = calculate_S_vib_total(x, n_list, Theta_E_list, T=T_dsdx, Theta_Li_scaled_100_times=True, style=style)
...
#         x = x_calculated[i]
#         x = x.requires_grad_()
#         s_tot, _, _ = calculate_S_config_total(x, S_config_params_list, style = "Legendre")
#         s_calculated[i] = s_tot
#         s_upper_bound_now, _, _ = calculate_S_config_total(x, [0.0], style = "Legendre")
...
#           fitted_entropy_omegas = None,
#           fitted_As = None,
#           fitted_Theta_Es = None,
#           T = 300):
#     """
...
#     fitted_entropy_omegas: fitted excess configurational entropy parameters, in the form of list, each element is nn.Parameter()
#     fitted_As: list of fitted weights for Einstein models, nn.Parameter()
#     fitted_Theta_Es: list of fitted effective Einstein temperatures, nn.Parameter()
#     T: temperature
...
#         if fitted_entropy_omegas == None:
#             sample = sampling(GibbsFE, params_list, T=T, sampling_id=1)
#         else:
#             sample = sampling(GibbsFE, [params_list, fitted_entropy_omegas, fitted_As, fitted_Theta_Es], T=T, sampling_id=1)
...
#                 else:
#                     common_tangent = CommonTangent(GibbsFE, [params_list, fitted_entropy_omegas, fitted_As, fitted_Theta_Es], T = T) # init common tangent model
...
#             phase_boundary_fixed_point = []
#         if fitted_entropy_omegas == None:
#             loss_collocation = alpha_collocation * collocation_loss_all_pts(mu, x, phase_boundary_fixed_point, GibbsFE, params_list, alpha_miscibility, T=T)
#         else:
#             loss_collocation = alpha_collocation * collocation_loss_all_pts(mu, x, phase_boundary_fixed_point, GibbsFE, [params_list, fitted_entropy_omegas, fitted_As, fitted_Theta_Es], alpha_miscibility, T=T)
#         # backprop
...
#                 x_now = x_now.requires_grad_()
#                 if fitted_entropy_omegas == None:
#                     g_now = GibbsFE(x_now, params_list, T=T)
#                 else:
#                     g_now = GibbsFE(x_now, [params_list, fitted_entropy_omegas, fitted_As, fitted_Theta_Es], T=T)
#                 mu_pred_now = autograd.grad(outputs=g_now, inputs=x_now, create_graph=True)[0]
...
#                     if fitted_entropy_omegas == None:
#                         ct_pred = (GibbsFE(x_alpha, params_list, T=T) - GibbsFE(x_beta, params_list, T=T))/(x_alpha - x_beta) 
#                     else:
#                         ct_pred = (GibbsFE(x_alpha, [params_list, fitted_entropy_omegas, fitted_As, fitted_Theta_Es], T=T) - GibbsFE(x_beta, [params_list, fitted_entropy_omegas, fitted_As, fitted_Theta_Es], T=T))/(x_alpha - x_beta) 
...
#           is_constrain_Theta_LiHM_1 = False,
#           Theta_LiHM_1_measured = 0.0,
#           alpha_Theta_LiHM_1 = 0.01,
#           ):
#     """
...
#     is_constrain_Theta_LiHM_1: whether we construct a loss function to make sure Theta_LiHM at x(Li)=1 matches experimental measured Debye temperature
#     Theta_LiHM_1_measured: experimental measured Debye temperature Theta_LiHM at x(Li)=1
#     alpha_Theta_LiHM_1: weight of Theta_LiHM1 constraint
...
#         assert len(pretrained_value_of_Theta_LiHM_polynomial_expasion) == order_of_Theta_LiHM_polynomial_expansion + 1
...
#         Theta_HM = nn.Parameter( torch.from_numpy(np.array([200.0/100],dtype="float32")) )# Theta_Li is scaled by 100 time
#     else:
...
#     """ 
#     Note that Theta_LiHM should be exactly the same as Theta_HM!!!!!
#     Because we can look up Theta_HM from literature (Debye temperature of HM and then convert to Einstein tempearature),
#     we don't train Theta_LiHM[0]
#     """
#     assert torch.abs(Theta_LiHM[0]-Theta_HM)<1e-4
#     for j in range(1, len(Theta_E_list[0])):
...
#         # dsdx file
#         if datafile_dsdx_name != None:
#             loss_entropy, _loss_dsdx, _loss_s_config_leq_0, _loss_s_config_geq_upper_bound = calc_loss_entropy(x_measured, dsdx_measured, S_config_params_list, n_list, Theta_E_list, T_dsdx=T_dsdx, style = polynomial_style)
#             loss = loss + alpha_dsdx * loss_entropy 
#         else:
#             loss_entropy = 0
#         # constraint on Theta_LiHM at x(Li)=1
#         if is_constrain_Theta_LiHM_1 == True:
#             x_comp = torch.from_numpy(np.array([1.0]).astype("float32"))
#             Theta_LiHM_calculated = calculate_Einstein_temperature_of_LixHM(x_comp, Theta_LiHM, Theta_LiHM_scale_100_times = True, style = "Legendre")
#             loss_ThetaHM_1 = (Theta_LiHM_calculated/10 - Theta_LiHM_1_measured/10)**2
#             loss = loss + loss_ThetaHM_1*alpha_Theta_LiHM_1
#         else:
#             loss_ThetaHM_1 = 0.0
#         # backprop & update
#         loss.backward()
...
#         epoch_record.append(epoch)
#         # print output
#         output_txt = "Epoch %3d  Loss %.4f  OCV1 %.4f  OCV2 %.4f  OCV3 %.4f  dSdx_col %.4f s<0 %.4f s>smax %.4f ThetaLiHM_1 %.4f     " %(epoch, loss, loss_collocation_1, loss_collocation_2, loss_collocation_3, _loss_dsdx, _loss_s_config_leq_0, _loss_s_config_geq_upper_bound, loss_ThetaHM_1)
#         # H excess params
#         for i in range(0, len(params_list)-1):
#             output_txt = output_txt + "Omega%d %.4f "%(i, params_list[i].item())
#         output_txt  = output_txt + "G0 %.4f "%(params_list[-1].item())
